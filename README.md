...
- **Lifecycle and handovers**: A Worker returns `review-ready`, which is a review input, not acceptance or completion: the Lead inspects the actual diff and evidence and decides `accepted` or `rejected`. A rejected result returns exactly one same-scope correction round, then a fresh Worker after approval. `handover` is the only terminal status - it ends the outgoing agent, and a fresh subagent is also required after acceptance, changed scope, corrections, or further work. `review-ready`, `checkpoint`, `blocked`, and `decision-needed` are pause reports: the Worker resumes its same unfinished unit once the Lead answers or the condition clears. Reports and handovers return curated comprehensive chat reports, not persisted files or exhaustive transcripts.
- **Circuit breakers**: attempts, correction rounds, and independent reviews are counted against the semantic work unit, not the agent, so the count survives Worker replacement, Lead succession, handovers, and relabeled briefs. A third attempt on a unit, a second correction round, a second independent review, failures that change location but not class, or growing verification volume with no acceptance criterion met stops implementation and escalates one tier with a loop report. A reset changes kind - reduced or split scope, changed approach, frozen partial acceptance, or a parked unit; another patch, Worker, or review pass is not a reset.
