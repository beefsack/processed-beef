...
on. The `150000` limit is skill-enforced: nearing its return threshold, or
when the next unit may exceed the remaining budget, a role reports `handover`
and stops - through chat when a live parent exists, otherwise as
`handover.md`.
